...
`tools/build-vhlt.sh` clones (if missing) and builds [VHLT-V34](https://github.com/twhl-community/VHLT-V34) — the `hlcsg`/`hlbsp`/`hlvis`/`hlrad`/`ripent` map compiler suite needed to compile `.map` sources into `.bsp` for this SDK's maps. It applies patches from `patches/vhlt/` (same idempotent apply-and-skip mechanism as `patches/xash3d/`, see below) right after cloning/updating, before running `make`. It's a plain `make`-based Linux build (`tools/VHLT-V34/src/zhlt-vluzacn/Makefile`), no CMake/waf involved. Outputs land in `tools/VHLT-V34/src/zhlt-vluzacn/bin/`. `./tools/build-vhlt.sh clean` rebuilds from scratch, `./tools/build-vhlt.sh update` pulls upstream first. Like `xash3d-fwgs/`, `tools/VHLT-V34/` is a git-ignored clone, not committed — the actual, persistent modification lives in `patches/vhlt/`.

`patches/vhlt/0001-hlrad-incremental-geometry-checksum.patch` fixes a real gap in `hlrad`'s `-incremental` transfer cache (`hlrad/transfers.cpp`): stock VHLT only invalidates the cached `.inc` file when the *patch count* changes, so a brush move that happens to keep the same patch count is silently accepted and produces stale bounce lighting on the new geometry, with no warning. The patch adds `GeometryChecksum()` — an FNV-1a hash over every patch's `origin`/`area`/`faceNumber` — written into the `.inc` header and re-verified on read; a mismatch now logs `Incremental file [...] is stale (geometry changed), recomputing` and correctly falls back to a full recompute. Verified by hand-corrupting just the checksum bytes of a valid `.inc` and confirming it's rejected while an unmodified file is still accepted.

`tools/build-maps.sh` compiles every `.map` in `mod/featureful/maps/*.map` (or a single one by name, e.g. `./build-maps.sh featureful_demo`) into a `.bsp` in place, via `hlcsg -> hlbsp -> hlvis -> hlrad` (`./build-maps.sh full <name>` for full vis/extra rad/bounce quality instead of the fast dev-loop defaults). It runs with `WADROOT=mod` so `hlcsg` resolves the wad paths baked into each `.map` (recorded from whatever machine authored the map, e.g. `/home/someone/.../Half-Life/valve/halflife.wad`) against `mod/valve/*.wad` instead — VHLT's `hlcsg` retries `$WADROOT/<original-wad's-parent-dir-name>/<wadfile>` when the literal embedded path doesn't exist. It also seeds `mod/valve/zhlt.wad` from `tools/VHLT-V34/tools/zhlt.wad` the first time, since maps expect it alongside the other wads. `hlrad` runs with `-incremental`, caching the transfer matrix in `<mapname>.inc` next to the `.bsp`/`.map` — note this cache only exists at all when bounces are enabled (`-bounce 0`, the "fast" default, skips the whole radiosity pass and never touches `.inc`); it only pays off with `full` mode or a custom `hlrad`-only re-run. **Important:** `hlcsg` unconditionally deletes any existing `.inc` on every *non*-`-onlyents` run (`common/log.cpp: ResetTmpFiles()`) — so under the normal full `hlcsg -> hlbsp -> hlvis -> hlrad` chain this script always runs, the cache never survives to the next `build-maps.sh` invocation anyway. The patch above only matters for the true entities-only workflow (`hlcsg -onlyents` skips `ResetTmpFiles`) or manual/non-standard build chains — `build-maps.sh` doesn't (yet) offer an entities-only fast path.
