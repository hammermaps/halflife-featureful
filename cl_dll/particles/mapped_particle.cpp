...
void CMappedParticle::Draw( void )
{
	...
	if (iNumFrames)
		flInverse = 1.0f / sqrt((float)iNumFrames);
...
bool CMappedParticle::Test( void )
{
	...
	if (pSys->iParticleCollision != PARTICLE_PASS_THROUGH) {
		...
		if ((bCollX == true || bCollY == true || bCollZ == true) && (pSys->iParticleCollision == PARTICLE_STUCK)) {
			...
			{
				sParticle.vVelocity.x = gEngfuncs.pfnRandomFloat( (0.01f*(cosf(10*sParticle.vPosition.y)+1)), (0.3f*(sinf(10*sParticle.vPosition.x)+1))); 
				sParticle.vVelocity.y = gEngfuncs.pfnRandomFloat( (0.01f*(cosf(10*sParticle.vPosition.y)+1)), (0.3f*(cosf(10*sParticle.vPosition.y)+1))); 
				sParticle.vVelocity.z = gEngfuncs.pfnRandomFloat( 0.01f, (0.3f*cosf(10*sParticle.vPosition.y)+1));
		...
		if((bCollX == true || bCollY == true || bCollZ == true) && pSys->iParticleCollision == PARTICLE_SPLASH) {
			if(gEngfuncs.PM_PointContents(sParticle.vPosition, NULL) == CONTENTS_WATER) {
